...
f <- list.files("data/Pisa_2021", pattern = "HAXO-", full.names = T, recursive = T)

fi <- data.frame(file = f,
                 site = parse_number(unlist(lapply(f, function(x) strsplit(x, "/")[[1]][3]))))

f <- list.files("data/Pisa21/HAXOT", pattern = "Pisa_", full.names = T, recursive = T)
...
df2 %>% mutate(haxo_probl = ifelse(site == 51, 0, haxo_probl)) -> df2
